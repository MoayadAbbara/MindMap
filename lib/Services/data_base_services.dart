...
class DatabaseService {
  ...
  return _db.collection("Subject").orderBy("timestamp" , descending: false).snapshots().map((snapshot) {
...
Future<void> changeProgress(SubjectModel subject ,bool isNew ,bool increment) async {
  if(increment & isNew) {
    _db.collection('Subject').doc(subject.id).update({
      'allLessonNum' : ++subject.allLessonNum,
      'progress' : (subject.completedLessonsNum / subject.allLessonNum *100).toInt()
      } 
    ); 
  }
...
Future<void> addLesson(int weekNum , String title , String subjectID) async{
  ...
  return _db.collection("Lesson").where('subjectID' ,isEqualTo: subjectID).snapshots().map((snapshot) {
    List<LessonModel> lessonList = [];
    for (DocumentSnapshot doc in snapshot.docs) {
      LessonModel lesson = LessonModel(title: doc['title'], weekNum: doc['weekNum']);
...
Future<void> checkLesson(String lessonID , bool isChecked) async{
  _db.collection('Lesson').doc(lessonID).update({
    'isDone' : isChecked
  });
}
